...
const CollectionTable = ({
  ...
  const autoGenerateCollectionDataInAllProducts = (
    ...
    if (!settingData.parentId && settingData.categoryId !== allProduct.categoryId) {
      if (event === 'add') {
        ...
        autoGenerateCollectionData = [{
          ...allProduct,
          collections: hasAllProductInCollection?.name ? allProduct.collections
            .filter(
      ...
      updateDropsCategory({
        variables: {
          CreateDropsCategoryForFront: {
            ...
            categoryData: autoGenerateCollectionData,
            collectionUpdateMsg: '',
          },
        },
      }).then(() => { }).catch((err) => {
  ...
  const handleAddCollectionModal = (data: any) => {
    if (data) {
      ...
      const autoDitectedAllProducts = FinalCollectionData
        .find((el) => el.type === CollectionType.ALLPRODUCT);
      autoGenerateCollectionDataInAllProducts('add', autoDitectedAllProducts);
    } else {
  ...
  const remove = (data: any) => {
    if (data === 'delete') {
      ...
      const autoDitectedAllProducts = collection
        .find((el) => el.type === CollectionType.ALLPRODUCT);
      autoGenerateCollectionDataInAllProducts('remove', autoDitectedAllProducts);
